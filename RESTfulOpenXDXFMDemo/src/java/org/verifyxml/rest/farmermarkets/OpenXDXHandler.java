 ...
 */
package org.verifyxml.rest.farmermarkets;
...
/**
 * Sample OpenXDX handler class for Farmer Markets Search Demo. Provides support for invocation of OpenXDX API calls
 * 
...
public class OpenXDXHandler {
    ...
    private static String FARM_MARKET_DATASOURCE_CONTEXT = "jdbc/FARMMARKET";

    // OpenXDX variables
    private static final String OPEN_XDX_DUMP_XML = "-open-xdx-dump.xml";

    //CAM Templates
    private static final String FARM_MARKET_QUERY_TEMPLATE =
        "/FMLookup.cxf";
    public File farmMarketTemplateFile;

    
    // Data source
    private DataSource datasourceFarmMarket;
    ...
    public OpenXDXHandler() {        
        ...
        try {
            ...
            datasourceFarmMarket = (DataSource)initialContext.lookup(FARM_MARKET_DATASOURCE_CONTEXT);
            farmMarketTemplateFile = extractResource(FARM_MARKET_QUERY_TEMPLATE);
        } catch (Exception e) {
    ...
    public String getOpenXDX(File cxfFile, Map<String, String> tokens) {
        ...
        try {
            ...
            // Set Data source
            processor.setDataSource(datasourceFarmMarket);
    ...
    protected void finalize() throws Throwable {        
        try {
            farmMarketTemplateFile.deleteOnExit();
        } finally {
            super.finalize();
        }
    }   
